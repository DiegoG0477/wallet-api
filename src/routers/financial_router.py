...
async def crear_meta_ahorro(meta_data: MetaAhorroCreate, request: Request):
    ...
    return await financial_controller.crear_meta_ahorro(usuario_id, meta_data)

@router.patch("/metas/{meta_id}", response_model=dict, dependencies=[Depends(auth_middleware)])
async def modificar_meta_ahorro(meta_id: str, meta_data: MetaAhorroCreate, request: Request):
    """
    Modificar una meta de ahorro existente.
    """
    usuario_id = request.state.user.id
    return await financial_controller.modificar_meta_ahorro(usuario_id, meta_id, meta_data)
...
async def eliminar_meta_ahorro(meta_id: str, request: Request):
    ...
    """
    usuario_id = request.state.user.id
    return await financial_controller.eliminar_meta_ahorro(usuario_id, meta_id)
...
async def obtener_metas_ahorro(request: Request):
    ...
    """
    usuario_id = request.state.user.id
    return await financial_controller.obtener_metas_ahorro(usuario_id)
...
async def crear_categoria_gasto(categoria_data: CategoriaGastoCreate, request: Request):
    ...
    """
    usuario_id = request.state.user.id
    return await financial_controller.crear_categoria_gasto(usuario_id, categoria_data)

@router.get("/categorias", response_model=List[CategoriaGastoGet], dependencies=[Depends(auth_middleware)])
async def obtener_categorias_gasto(periodo: float, request: Request):
    """
    Obtener todas las categorías de gasto de un usuario y calcular el total gastado por categoría.
    """
    usuario_id = request.state.user.id
    return await financial_controller.obtener_categorias_gasto(usuario_id, periodo)
...
async def registrar_gasto(gasto_data: GastoCreate, request: Request):
    ...
    """
    usuario_id = request.state.user.id
    return await financial_controller.registrar_gasto(usuario_id, gasto_data)
...
async def abonar_meta(meta_id: str, monto: float, request: Request):
    ...
    """
    usuario_id = request.state.user.id
    return await financial_controller.abonar_meta(usuario_id, meta_id, monto)